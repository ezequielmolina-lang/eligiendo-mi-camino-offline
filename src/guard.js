...
const SELF_HARM = /(no quiero (mas )?vivir|ya no.{0,8}vivir|quiero morir|me quiero morir|quiero matarme|\bmatarme\b|suicid|quitarme la vida|acabar con (mi vida|todo)|hacerme dano|lastimarme|me quiero cortar|cortarme( las venas)?|no.{0,6}sentido a (la vida|nada|vivir)|estaria mejor muerto|nadie me (quiere|extranaria)|mejor desaparezco|desaparecer para siempre|ya no aguanto( mas)?( la vida)?|kill myself|wanna die|want to die|hurt myself|end my life|no reason to live|commit suicide)/;
...
const DISTRESS = /(me siento (muy )?(triste|solo|sola|deprimid|vac[ií]o|fatal|abajo|in[uú]til)|estoy (muy )?(triste|deprimid|bajonead|destrozad)|ando (triste|deprimid|bajonead)|me siento mal conmigo|tengo ganas de llorar|ganas de llorar|no tengo ganas de nada|sin ganas de nada|nadie me entiende|me siento un fracaso|todo me sale mal y me siento)/;
...
export function guardInput(text) {
  const t = norm(text);
  if (SELF_HARM.test(t) || ABUSE.test(t)) return { block: true, kind: 'crisis', response: R_CRISIS };
